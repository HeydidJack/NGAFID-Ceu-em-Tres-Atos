#!/usr/bin/env python3
"""
KEL for Fault Classification (FC)
==================================
Knowledge Distillation with Keyness Extraction for multi-class fault classification.
Filters healthy samples, applies class-balanced data augmentation, and extracts temporal keyness.

Usage:
    python exp_kel_fc.py
"""
...
from configs.fc_kd_config import get_args, get_args_meanings
...
def LoadDataFolds(data_select_pattern="2days"):
    ...
    for fold_id in range(5):
        ...
        health_index = 0
        anomaly_mask = fold_label[:, health_index] == 0.0

        if np.sum(anomaly_mask) == 0:
            raise ValueError(f"Fold {fold_id} contains no anomalous samples after filtering!")

        filtered_data = fold_data[anomaly_mask]
        # Remove health column, keep fault classes only
        filtered_label = fold_label[anomaly_mask, 1:]
...
class Exp_KEL_FC(Exp_Basic):
    """KEL Training Experiment for Fault Classification."""

    def __init__(self, args):
        super(Exp_KEL_FC, self).__init__(args)
    ...
    def _get_data(self, fold_index_list, fold_data_list, fold_label_list, test_fold_id=0, scaler_save_path=None):
        """Prepare data with class-balanced augmentation (FC specific)."""
        self.num_folds = 5
        ...
        # Data Augmentation for minority classes (FC critical)
        if self.args.da_this_exp:
            ...
            for class_idx in range(train_label_split.shape[1]):
                class_mask = train_label_split[:, class_idx] == 1
                class_data = train_data_split[class_mask]
                class_labels = train_label_split[class_mask]
                class_index = train_index_split[class_mask]

                num_to_augment = int(max_count - len(class_data))
                if num_to_augment > 0 and len(class_data) > 0:
                    # Use DA Factory - consistent with exp_fc_classification.py
                    augmentor = DataAugmentationFactory.get_augmentation_method(self.args.da_method)(
                        self.args.num_copies)
                    aug_data, aug_labels = augmentor.augment(class_data, class_labels)

                    # Take only needed amount
                    aug_data = aug_data[:num_to_augment]
                    aug_labels = aug_labels[:num_to_augment]

                    # Generate new indices
                    start_idx = np.max(train_index_split) + 1 if len(train_index_split) > 0 else 0
                    aug_index = np.arange(start_idx, start_idx + len(aug_data))

                    augmented_data.append(aug_data)
                    augmented_labels.append(aug_labels)
                    augmented_index.append(aug_index)
    ...
    def train(self, setting):
        self._init_model()
        path = os.path.join(f"{PROJECT_ROOT}/ModelCheckpoints", self.args.checkpoints, "KEL_FC", setting)
...
if __name__ == "__main__":
    ...
    Exp_log_dir, Exp_log_path, Exp_outcome_path = init_exp_log(
        ...
        data_info=args.data,
        task_type=f"KEL_FC_{data_select_pattern}"
    )
