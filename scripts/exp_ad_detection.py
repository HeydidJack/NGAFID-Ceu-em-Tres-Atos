...
=================================
End-to-end training script for Anomaly Detection (AD) task.
Maintains original logic flow; only infrastructure imports updated.

Usage:
    python exp_ad_detection.py

    # Or import for programmatic use:
    from exp_ad_detection import Exp_AD
...
# Add project root for importing tools
PROJECT_ROOT = Path(__file__).parent.parent.resolve()
sys.path.insert(0, str(PROJECT_ROOT))

# Updated imports: using new tool infrastructure
from tools.exp_logger import init_exp_log, print_aptxt, append_text_to_file, get_divider, get_current_datetime_string
from configs.ad_config import get_args, get_args_meanings

# Global log paths (maintaining original pattern for compatibility)
Exp_log_path = None
...
def check_and_generate_data(output_folder=f"{PROJECT_ROOT}\ProcessedData\ProcessedData_Diagnosis_DeD_2days"):
    """
    Verify processed dataset existence.
    Terminate immediately if missing (do not auto-generate to avoid accidental long-running preprocessing during training).
    """
    if not os.path.exists(output_folder):
        print(f"[FATAL] Processed data directory not found: {output_folder}")
        print("\nPlease generate the dataset first using:")
        print(f"  python scripts/prepare_diagnosis_data.py --dataset {'2days' if '2days' in output_folder else 'full'}")
...
def LoadDataFolds(data_select_pattern="2days"):
    ...
    Converts multi-class diagnosis labels to binary AD labels (Healthy vs Anomaly).
    """
    fold_data_list = []
    fold_label_list = []
    fold_index_list = []
    ...
    for fold_id in range(5):
        ...
        # Convert diagnosis labels to binary AD labels
        # Assuming class 0 is "Healthy" (flights recorded after maintenance)
        health_index = 0
        ...
        print_aptxt(
            f'Fold {fold_id}: data shape {fold_data.shape}, label shape {new_label.shape} loaded at {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}',
            Exp_log_path
        )
...
class Exp_AD(Exp_Basic):
    """Anomaly Detection Experiment Class."""
    ...
    def _check_model_arcsize(self):
        """Calculate and log model parameter count and memory usage."""
    ...
    def _get_data(self, fold_index_list, fold_data_list, fold_label_list, test_fold_id=0):
        ...
        # Concatenate 4 folds for training
        ...
        # Create data loaders (list of batches)
        ...
        # Prepare test set using same scaler
    ...
    def _predict(self, batch_x):
        """Model forward pass with optional AMP."""

        def _run_model():
            outputs = self.model(batch_x)
            if isinstance(outputs, tuple):  # Handle models returning (output, attention)
                outputs = outputs[0]
            return outputs
    ...
    def calculate_metrics(self, predictions, labels):
        """Calculate classification metrics including FNR (False Negative Rate)."""
    ...
    def train(self, setting, continue_training=None):
        ...
        if continue_training:
            train_loader, vali_loader, test_loader = continue_training.train_loader, continue_training.vali_loader, continue_training.test_loader
            self.train_loader, self.vali_loader, self.test_loader = train_loader, vali_loader, test_loader
        ...
        self.model = self._build_model().to(self.device)
        if continue_training:
            self.model.load_state_dict(torch.load(continue_training.model_path))
        ...
        for epoch in range(self.args.train_epochs):
            ...
            print_aptxt(f"Fold: {self.args.testfoldid} Epoch: {epoch + 1} cost time: {epoch_time}", Exp_log_path)
            train_loss = np.average(train_loss)
            vali_loss = self.vali(vali_loader, criterion)
            test_loss = self.vali(test_loader, criterion)

            print_aptxt(
                f"Fold: {self.args.testfoldid} Epoch: {epoch + 1}, Steps: {train_steps} | "
    ...
    def test_clas(self):
        """Testing and inference efficiency measurement."""
...
if __name__ == "__main__":
    ...
    Exp_log_dir, Exp_log_path, Exp_outcome_path = init_exp_log(
        ...
        data_info=args.data,
        task_type=f"AD_{data_select_pattern}"
    )
    ...
    # Data loading (global scope for legacy compatibility)
