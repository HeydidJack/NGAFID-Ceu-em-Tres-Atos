#!/usr/bin/env python3
"""
Fault Classification Training Script
====================================
Multi-class fault classification on anomalous samples only.
Utilizes data augmentation to address class imbalance.
...
def LoadDataFolds(data_select_pattern="2days"):
    """
    Load 5-fold CV data for Fault Classification.
    CRITICAL: Filters out healthy samples (health_index = 0), keeping only anomalous flights.
    """
    fold_data_list = []
    fold_label_list = []
    fold_index_list = []
    ...
    for fold_id in range(5):
        ...
        fold_label = np.array(fold_dict['label'], dtype=np.float32)  # (N, num_classes)
        fold_indexes = fold_dict['index']

        # Filter: Remove healthy samples (health_index = 0 in one-hot)
        health_index = 0
        anomaly_mask = fold_label[:, health_index] == 0.0  # Keep only non-healthy

        if np.sum(anomaly_mask) == 0:
            raise ValueError(f"Fold {fold_id} contains no anomalous samples after filtering!")

        filtered_data = fold_data[anomaly_mask]
        filtered_label = fold_label[anomaly_mask, 1:]  # Remove health column, keep fault classes
        filtered_indexes = np.array(fold_indexes)[anomaly_mask]

        # Renormalize labels to one-hot without health class
        # If original was [0,1,0,0] (health+3 faults), new becomes [1,0,0] (3 faults)
        # Note: This assumes health is always index 0

        fold_data_list.append(filtered_data)
        fold_label_list.append(filtered_label)
        fold_index_list.append(filtered_indexes)

        print_aptxt(
            f'Fold {fold_id}: {np.sum(anomaly_mask)} anomalous samples loaded '
            f'(filtered out {len(fold_data) - np.sum(anomaly_mask)} healthy)',
            Exp_log_path
        )

    print_aptxt("FC Data loading completed (healthy samples excluded)\n" + get_divider(), Exp_log_path)
...
class Exp_FC(Exp_Basic):
    """Fault Classification Experiment Class."""

    def __init__(self, args):
        super(Exp_FC, self).__init__(args)
    ...
    def calculate_metrics(self, predictions, labels):
        ...
        # Macro average for multi-class (treats all classes equally)
    ...
    def train(self, setting):
        """Main training loop for Fault Classification."""
        ...
        path = os.path.join(f"{PROJECT_ROOT}/ModelCheckpoints", self.args.checkpoints, "FC_" + self.args.data_select_pattern, setting)
    ...
    def test_recorder(self, metrics, target_file_path, is_print_out=True):
        if is_print_out:
            ...
            print_aptxt(f"Precision (Macro): {metrics.get('precision', 0):.4f}", target_file_path)
            print_aptxt(f"Recall (Macro): {metrics.get('recall', 0):.4f}", target_file_path)
            ...
            append_text_to_file(f"Precision (Macro): {metrics.get('precision', 0):.4f}", target_file_path)
            append_text_to_file(f"Recall (Macro): {metrics.get('recall', 0):.4f}", target_file_path)
...
if __name__ == "__main__":
    ...
    Exp_log_dir, Exp_log_path, Exp_outcome_path = init_exp_log(
        ...
        data_info=args.data,
        task_type=f"FC_{data_select_pattern}"
    )
