#!/usr/bin/env python3
"""
End-to-End Diagnosis Training Script
====================================
Unified health diagnosis combining Anomaly Detection and Fault Classification.
Uses full label space and evaluates with MCWPM (Multi-Class Weighted Penalty Metric).
...
def check_and_generate_data(output_folder=f"{PROJECT_ROOT}\ProcessedData\ProcessedData_Diagnosis_DeD_2days"):
    ...
    if not os.path.exists(output_folder):
        ...
        print("\nFor manual setup instructions, see DATASET.md")
        sys.exit(1)
    ...
    if len(pkl_files) == 0:
        ...
        print("The folder appears to be empty or corrupted.")
...
def LoadDataFolds(data_select_pattern="2days"):
    """
    Load 5-fold CV data for End-to-End Diagnosis.
    CRITICAL: Uses FULL labels including healthy class (index 0).
    No filtering applied - end-to-end uses complete label space.
    """
    fold_data_list = []
    fold_label_list = []
    fold_index_list = []
    ...
    for fold_id in range(5):
        ...
        fold_data = np.array(fold_dict['train'], dtype=np.float32)  # (N, 2048, 23)
        fold_label = np.array(fold_dict['label'], dtype=np.float32)  # (N, num_classes) - full one-hot
        fold_indexes = fold_dict['index']

        # Diagnosis uses full labels (healthy + all faults) - NO FILTERING
        fold_data_list.append(fold_data)
        fold_label_list.append(fold_label)
        fold_index_list.append(fold_indexes)

        # Log class distribution for verification
        health_mask = fold_label[:, 0] == 1.0
        healthy_count = np.sum(health_mask)
        fault_count = len(fold_label) - healthy_count

        print_aptxt(
            f'Fold {fold_id}: {len(fold_data)} total samples '
            f'(Healthy: {healthy_count}, Faults: {fault_count}, '
            f'Classes: {fold_label.shape[1]})',
            Exp_log_path
        )

    print_aptxt("Diagnosis Data loading completed (full label space)\n" + get_divider(), Exp_log_path)
...
class Exp_Diagnosis(Exp_Basic):
    """End-to-End Diagnosis Experiment Class."""

    def __init__(self, args):
        super(Exp_Diagnosis, self).__init__(args)
        self.have_got_dataset = False
        self.scaler = None

    def _check_model_arcsize(self):
        print_aptxt(get_divider() + "Model Size Information:", Exp_log_path)
        example_model = self.args.model(self.args)
    ...
    def _get_data(self, fold_index_list, fold_data_list, fold_label_list, test_fold_id=0):
        """Prepare train/val/test with StandardScaler and optional Data Augmentation."""
        self.num_folds = 5
        assert 0 <= test_fold_id < self.num_folds

        # Concatenate 4 training folds
        ...
        if self.args.da_this_exp:
            label_counts = np.sum(train_label_split, axis=0)
            max_count = np.max(label_counts)
            print_aptxt(f"Original training class distribution: {label_counts}", Exp_log_path)

            augmented_data = []
            augmented_labels = []
            augmented_index = []
            for class_idx in range(train_label_split.shape[1]):
                class_data = train_data_split[train_label_split[:, class_idx] == 1]
                class_labels = train_label_split[train_label_split[:, class_idx] == 1]
                class_index = train_index_split[train_label_split[:, class_idx] == 1]

                num_to_augment = int(max_count - len(class_data))
                if num_to_augment > 0:
                    augmentor = DataAugmentationFactory.get_augmentation_method(self.args.da_method)(
                        self.args.num_copies - 1)
                    augmented_class_data, augmented_class_labels = augmentor.augment(class_data, class_labels)

                    augmented_class_index = []
                    for i in range(min(len(augmented_class_data), num_to_augment)):
                        da_idx = max(class_index) + i
                        augmented_class_index.append(da_idx)

                    augmented_data.append(augmented_class_data[:num_to_augment])
                    augmented_labels.append(augmented_class_labels[:num_to_augment])
                    augmented_index.append(np.array(augmented_class_index))

            if augmented_data:
                augmented_data = np.concatenate(augmented_data, axis=0)
                augmented_labels = np.concatenate(augmented_labels, axis=0)
                augmented_index = np.concatenate(augmented_index, axis=0)

                train_data_split = np.concatenate([train_data_split, augmented_data], axis=0)
                train_label_split = np.concatenate([train_label_split, augmented_labels], axis=0)
                train_index_split = np.concatenate([train_index_split, augmented_index], axis=0)

            print_aptxt(f"Augmented training class distribution: {np.sum(train_label_split, axis=0)}", Exp_log_path)
        ...
        # Create loaders
        self.train_loader = self._make_loader(train_data_split, train_label_split, train_index_split)
        self.vali_loader = self._make_loader(val_data_split, val_label_split, val_index_split)
        self.test_loader = self._make_loader(test_data, test_label, test_index)

        self.scaler = scaler
        self.have_got_dataset = True

    def _make_loader(self, data, label, index):
        """Package into list of dotdict batches."""
    ...
    def calculate_metrics(self, predictions, labels):
        """
        Calculate diagnosis metrics including:
        - Standard: ACC, Macro-F1
        - Weighted: WF1 (Weighted F1 for imbalanced classes)
        - Safety: MCWPM (Multi-Class Weighted Penalty Metric with alpha/beta)
        """
        ...
        precision = precision_score(label_indices.cpu().numpy(), pred_indices.cpu().numpy(), average='macro',
                                    zero_division=0)
        recall = recall_score(label_indices.cpu().numpy(), pred_indices.cpu().numpy(), average='macro', zero_division=0)
        f1 = f1_score(label_indices.cpu().numpy(), pred_indices.cpu().numpy(), average='macro', zero_division=0)

        # Weighted F1 (accounts for class imbalance)
        wf1 = f1_score(label_indices.cpu().numpy(), pred_indices.cpu().numpy(), average='weighted', zero_division=0)

        # MCWPM calculation (treating healthy as negative, faults as positive for safety)
        mcwpm = self.calculate_mcwpm(predictions, labels, alpha=self.args.mcwpm_alpha, beta=self.args.mcwpm_beta)
    ...
    def calculate_mcwpm(self, predictions, labels, alpha=2.5, beta=1.0):
        """
        Multi-Class Weighted Penalty Metric (MCWPM).

        MCWPM = TP / (TP + alpha * FN_health + beta * FP_health)

        Where:
        - TP: Correctly identified samples (all classes)
        - FN_health: Fault samples misclassified as Healthy (Missed Detection)
        - FP_health: Healthy samples misclassified as Fault (False Alarm)

        Args:
            alpha: Penalty for missed anomaly (safety-critical, default 2.5)
            beta: Penalty for false alarm (cost-sensitive, default 1.0)
        """
        pred_indices = torch.argmax(torch.tensor(predictions), dim=1)
        label_indices = torch.argmax(torch.tensor(labels), dim=1)

        health_idx = self.args.health_index  # Typically 0

        # Calculate TP, FN_health, FP_health
        TP = torch.sum(pred_indices == label_indices).item()

        # FN_health: Actually fault (label != health) but predicted as health
        FN_health = torch.sum((label_indices != health_idx) & (pred_indices == health_idx)).item()

        # FP_health: Actually health but predicted as fault
        FP_health = torch.sum((label_indices == health_idx) & (pred_indices != health_idx)).item()

        numerator = TP
        denominator = numerator + alpha * FN_health + beta * FP_health

        mcwpm = numerator / denominator if denominator > 0 else 0.0
        return mcwpm
    ...
    def train(self, setting):
        """Main training loop for Diagnosis."""
        self._get_data(fold_index_list, fold_data_list, fold_label_list, self.args.testfoldid)

        total_train_start_time = time.time()
        epoch_times = []

        print_aptxt(f"Train samples: {len(self.train_loader)}", Exp_log_path)
        print_aptxt(f"Val samples: {len(self.vali_loader)}", Exp_log_path)
        print_aptxt(f"Test samples: {len(self.test_loader)}", Exp_log_path)

        path = os.path.join(f"{PROJECT_ROOT}/ModelCheckpoints", self.args.checkpoints, "Diagnosis_" + self.args.data_select_pattern,
                            setting)
        if self.args.save_model_path:
            path = os.path.join(f"{PROJECT_ROOT}/ModelCheckpoints", self.args.save_model_path)
        os.makedirs(path, exist_ok=True)

        print_aptxt(f"Model checkpoint path: {path}/checkpoint.pth", Exp_log_path)
        ...
        for epoch in range(self.args.train_epochs):
            ...
            self.model.train()
            for i, train_sam in enumerate(self.train_loader):
                iter_count += 1
                ...
                if (i + 1) % 200 == 0:
                    print_aptxt(
                        f"\titers: {i + 1}, epoch: {epoch + 1}, fold: {self.args.testfoldid} | loss: {loss.item():.7f}",
                        Exp_log_path
                    )
            ...
            print_aptxt(
                f"Fold: {self.args.testfoldid} Epoch: {epoch + 1} | "
    ...
    def test_clas(self):
        """Testing with inference time measurement."""
        if not self.have_got_dataset:
            self._get_data(fold_index_list, fold_data_list, fold_label_list, self.args.testfold_id)
        ...
        with torch.no_grad():
            for test_sam in self.test_loader:
                ...
                output = outputs.reshape(-1, self.args.clasnum)
            ...
            pred_out = torch.concat(pred_out, dim=0).reshape(-1, self.args.clasnum)
            true_out = torch.concat(true_out, dim=0).reshape(-1, self.args.clasnum)
    ...
    def test_recorder(self, metrics, target_file_path, is_print_out=True):
        """Log metrics including Diagnosis-specific WF1 and MCWPM."""
        if is_print_out:
            print_aptxt(f"ACC: {metrics.get('accuracy', 0):.4f}", target_file_path)
            print_aptxt(f"F1 (Macro): {metrics.get('f1_score', 0):.4f}", target_file_path)
            print_aptxt(f"WF1: {metrics.get('wf1', 0):.4f}", target_file_path)
            print_aptxt(f"MCWPM: {metrics.get('mcwpm', 0):.4f}", target_file_path)
            ...
            print_aptxt(f"Confusion Matrix:\n{metrics.get('confusion_matrix', 'N/A')}", target_file_path)
        else:
            append_text_to_file(f"ACC: {metrics.get('accuracy', 0):.4f}", target_file_path)
            append_text_to_file(f"F1 (Macro): {metrics.get('f1_score', 0):.4f}", target_file_path)
            append_text_to_file(f"WF1: {metrics.get('wf1', 0):.4f}", target_file_path)
            append_text_to_file(f"MCWPM: {metrics.get('mcwpm', 0):.4f}", target_file_path)
...
if __name__ == "__main__":
    ...
    Exp_log_dir, Exp_log_path, Exp_outcome_path = init_exp_log(
        ...
        data_info=args.data,
        task_type=f"Diagnosis_{data_select_pattern}"
    )

    current_datetime, _ = get_current_datetime_string()
    append_text_to_file(str(args_meanings), Exp_log_path)
    print_aptxt('Args in experiment:' + str(args), Exp_log_path)

    os.environ['CUDA_LAUNCH_BLOCKING'] = '1'

    # Load data (global scope for compatibility)
