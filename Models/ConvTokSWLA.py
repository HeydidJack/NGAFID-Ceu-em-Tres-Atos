...
class SlidingWindowAttention(nn.Module):
    """
    Local attention focusing only on w positions before and after (including itself, total 2*w+1 positions).
    If mask_flag=True, additionally stack causal mask (upper right triangle invisible).
    Other interfaces are completely consistent with FullAttention, can be directly replaced.
    """
    def __init__(self, window_size=3, mask_flag=False, scale=None,
                 attention_dropout=0.1, output_attention=False):
        super().__init__()
        self.w = window_size                      # Unilateral window length
    ...
    def forward(self, queries, keys, values, attn_mask=None):
        ...
        assert L == S, "SlidingWindowAttention requires L == S"
        ...
        # 1. Construct sliding window mask: only keep 2*w+1 tokens near diagonal
        band_mask = torch.ones(L, L, dtype=torch.bool, device=queries.device)
        # Lower triangle offset -w, upper triangle offset +w
        band_mask = torch.triu(band_mask, diagonal=-self.w)
        band_mask = torch.tril(band_mask, diagonal=self.w)
        window_mask = ~band_mask  # True indicates positions to be masked

        scores.masked_fill_(window_mask.unsqueeze(0).unsqueeze(0), -np.inf)

        # 2. If causal mask (upper right triangle) is needed, stack it additionally
        if self.mask_flag:
...
class ConvTokSWLA(nn.Module):
    def __init__(self, configs):
        super(ConvTokSWLA, self).__init__()
        self.L_patch = configs.L_patch
                ...
                EncoderLayer(
                    AttentionLayer(
                        SlidingWindowAttention(
                            window_size=configs.viewindow,  # Look 5 steps before and after, total 11 positions (2*w+1)
                            mask_flag=False,  # If task needs causal, set True
...
def get_args_meanings_dict():
    args_meanings_dict = {
        ...
        "viewindow": "Attention window size.",
