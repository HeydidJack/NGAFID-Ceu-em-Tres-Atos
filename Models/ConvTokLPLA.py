...
# =================== LogParse Sparse Compression ===================
class LogParseProjection(nn.Module):
    """
    Apply log(1+|x|) mapping + learnable sparse gate to keys within window;
    Input: x (..., w, d)
    Output: x' (..., w, d) values smoother, sparser, inter-head differences amplified
    """
    def __init__(self, d_k, eps=1e-6):
        super().__init__()
        self.log_scale = nn.Parameter(torch.ones(1))
        self.gate = nn.Linear(d_k, 1)   # Give each position a sparse gate weight
        self.eps = eps

    def forward(self, x):
        # x: (..., w, d)
        log_x = torch.log1p(torch.abs(x)) * self.log_scale
        sparse_w = torch.sigmoid(self.gate(x))          # (..., w, 1)
        return log_x * sparse_w                         # Broadcast back to (..., w, d)


# =================== Window + LogParse Attention ===================
class LogParseLocalAttention(nn.Module):
    def __init__(self, window_size=3, n_heads=4, mask_flag=False,
        ...
        self.scale = 1.0 / sqrt(self.w_size[0])  # First use first head window size for scale, can be refined later
    ...
    def forward(self, queries, keys, values, attn_mask=None):
        ...
        assert len(self.w_size) == H

        # 1. Local mask construction (same as MWLA)
        scores = torch.einsum("blhe,bshe->bhls", queries, keys)
        for h in range(H):
            w = self.w_size[h]
            if w == 0:  # Skip global head
                continue
        ...
        # 2. LogParse: apply sparse compression to keys within window
        # First get keys within window subset (using mask to extract)
        for h in range(H):
            w = self.w_size[h] if self.w_size[h] != 0 else L
            # Construct sliding window indices, simplified implementation: directly apply log-scale compensation to scores
            # Actual keys within window are already reflected in einsum, here only map keys
            keys_lp = keys[:, :, h]  # (B,S,d_k)
            proj = LogParseProjection(keys_lp.size(-1)).to(device)
            keys_lp = proj(keys_lp)  # (B,S,d_k)
            # Recalculate this head q·k'
            q_h = queries[:, :, h]   # (B,L,d_k)
            scores[:, h] = torch.einsum("bld,bsd->bls", q_h, keys_lp) * self.scale
...
class ConvTokLPLA(nn.Module):
    def __init__(self, configs):
        super(ConvTokLPLA, self).__init__()
        self.L_patch = configs.L_patch
        ...
        self.encoder = Encoder(
            [
                EncoderLayer(
                    AttentionLayer(
                        LogParseLocalAttention(
                            window_size=configs.viewindow_size,  # Still supports list/int
