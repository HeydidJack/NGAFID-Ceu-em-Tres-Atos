...
# Add KeynessModule before ConvTokMHSA model
class KEL_ConvTokMHSA(nn.Module):
    def __init__(self, configs, is_tokenized=False):
        super(KEL_ConvTokMHSA, self).__init__()
        self.keyness_module = KeynessModule(configs)
        self.tokenizermhsa = ConvTokMHSA(configs)

    def forward(self, x):
        # First pass through KeynessModule
        weighted_x, Keyness = self.keyness_module(x)
        # Then through ConvTokMHSA model
        output, encoder_output = self.tokenizermhsa(weighted_x)
...
class TimeKEL_ConvTokMHSA(nn.Module):
    def __init__(self, configs, is_tokenized=False):
        super(TimeKEL_ConvTokMHSA, self).__init__()
