...
class FullAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask):
        ...
        _, S, _, D = values.shape  # S is sequence length, D is dimension

        scale = self.scale or 1. / sqrt(E)  # Can choose normalization methods other than inverse square root of embedding dimension

        # torch.einsum is a very powerful function in PyTorch for performing Einstein summation convention tensor operations
        # It can flexibly perform tensor dot products, matrix multiplications, tensor contractions, etc., according to user-specified symbolic rules, even implementing some complex tensor transformations
        # Repeated indices indicate summation: if an index appears on both sides of the equation, sum over that index
        # Unrepeated indices indicate output tensor dimensions: unrepeated indices are retained in the output tensor
        scores = torch.einsum("blhe,bshe->bhls", queries, keys)

        if self.mask_flag:  # Used in Decoder to mask upper triangle of target attention score matrix
            if attn_mask is None:  # Used for special masking
                attn_mask = TriangularCausalMask(B, L, device=queries.device)

            scores.masked_fill_(attn_mask.mask, -np.inf)  # Use masked_fill_ method to fill all positions where mask matrix is True with negative infinity (-np.inf)

        A = self.dropout(torch.softmax(scale * scores, dim=-1))  # Process attention score matrix through softmax
...
class ConvTokMHSA(nn.Module):
    def __init__(self, configs):
        super(ConvTokMHSA, self).__init__()
        self.L_patch = configs.L_patch
...
def get_args():
    ...
    args.testfoldid = 1
    # Set model save path
    setting = '{}_{}_lp{}_td{}_nh{}_il{}_el{}_df{}'.format(
        args.model_name,
        args.data,
        args.L_patch,
        args.token_dim,
